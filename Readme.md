...
One question you might ask is: even if you increase the number of backend servers, doesn't the fact we only have a single Redis server mean we are bottlenecked? 
...
This project requires redis in order to run. To run locally, we suggest installing docker, and running `docker run -p 6379:6379 redis`. You also need to enable enable notifications in Redis. To do so, `docker exec` into your running container, run `redis-cli`, and then execute the command `CONFIG SET notify-keyspace-events AKE`. 
